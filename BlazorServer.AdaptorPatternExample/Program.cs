using BlazorServer.AdaptorPatternExample.Data;
...
builder.Services.AddDbContext<AdaptorPatternExampleContext>(
    opt => opt.UseSqlite(builder.Configuration.GetConnectionString("AdaptorPattern")));
...
async Task EnsureDbIsMigrated(IServiceProvider services)
{
    ...
    using var ctx = scope.ServiceProvider.GetService<AdaptorPatternExampleContext>();
    if (ctx != null)
    {
